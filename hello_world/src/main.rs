...
impl Bound{
    fn new_bound() -> Bound{
        Bound{min:vec![-1.0,-1.0],max:vec![2.0,1.0]}
    }
...
impl Particle{
    fn new_particle() -> Particle{
        ...
        Particle{position,velocity,local_best_position,local_best_loss,position_bound,velocity_bound}
    }
    ...
    fn evolution(&mut self, w:f64,c1:f64,c2:f64,global_best_position:&Vec<f64>){
        ...
        for i in 0..position_len{
            self.velocity[i]=w*self.position[i]+c1*rng.gen::<f64>()*(self.local_best_position[i]-self.position[i])+c2*rng.gen::<f64>()*(global_best_position[i]-self.position[i]);
...
impl Swarm{
    fn new_swarm(num_of_particles:usize)->Swarm{
        ...
        Swarm{particles,global_best_position,global_best_loss}
    }
...
fn main() {
    let mut a_swarm = Swarm::new_swarm(2000);
    for i in 0..10{
        a_swarm.evolution(0.5,4.0,2.0);
        println!("{:?}",&a_swarm.global_best_loss);
    }
